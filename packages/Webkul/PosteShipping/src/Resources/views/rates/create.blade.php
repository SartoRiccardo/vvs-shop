<x-admin::layouts>
    <x-slot:title>Add Rate — {{ $zone->name }}</x-slot>
    ...
    <div class="flex items-center gap-3">
        ...
        <p class="text-xl font-bold text-gray-800 dark:text-white">Add Rate — {{ $zone->name }}</p>
    </div>

    <div class="mt-6 max-w-sm rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <form action="{{ route('services.zones.rates.store', [$service, $zone]) }}" method="POST" class="space-y-4">
            @csrf

            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Max weight (kg)</label>
                <input type="number" name="max_weight_kg" value="{{ old('max_weight_kg') }}" step="0.001" min="0.001" required
            ...
            <div>
                ...
                <input type="number" name="cost_eur" value="{{ old('cost_eur') }}" step="0.01" min="0" required
                    class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white" />
