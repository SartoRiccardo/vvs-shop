<x-admin::layouts>
    <x-slot:title>Edit Service</x-slot>

    <div class="flex items-center gap-3">
        <a href="{{ route('services.index') }}" class="text-gray-500 hover:text-gray-700 dark:text-gray-400">← Services</a>
        <p class="text-xl font-bold text-gray-800 dark:text-white">Edit — {{ $service->name }}</p>
    </div>
    ...
    <div class="mt-6 max-w-lg rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <form action="{{ route('services.update', $service) }}" method="POST" class="space-y-4">
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="active" id="active" value="1" {{ old('active', $service->active) ? 'checked' : '' }} class="rounded" />
