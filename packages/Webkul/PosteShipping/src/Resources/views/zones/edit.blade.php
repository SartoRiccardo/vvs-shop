<x-admin::layouts>
    <x-slot:title>Edit Zone — {{ $zone->name }}</x-slot>
    ...
    <div class="flex items-center gap-3">
        ...
        <p class="text-xl font-bold text-gray-800 dark:text-white">Edit Zone — {{ $zone->name }}</p>
    </div>

    <div class="mt-6 max-w-2xl rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <form action="{{ route('services.zones.update', [$service, $zone]) }}" method="POST" class="space-y-4">
            @csrf @method('PUT')
