        ...
        @if (! $addresses->isEmpty())
            ...
            <div class="mt-[60px] grid grid-cols-2 gap-5 max-1060:grid-cols-[1fr] max-md:mt-5">
                @foreach ($addresses as $address)
                    <div class="p-5 border rounded-xl border-divider max-md:flex-wrap">
                        <div class="flex justify-between">
                            ...
                            <div class="flex gap-4 max-sm:gap-2.5">
                                ...
                                <x-shop::dropdown position="bottom-{{ core()->getCurrentLocale()->direction === 'ltr' ? 'right' : 'left' }}">
                                    <x-slot:toggle>
                                        <button 
                                            class="icon-more cursor-pointer rounded-md px-1.5 py-1 text-2xl text-mutedText transition-all hover:bg-gray-100 hover:text-black focus:bg-gray-100 focus:text-black max-md:p-0" 
                        ...
                        <p class="mt-6 text-mutedText max-md:mt-5 max-md:text-sm" v-pre>
                            {{ $address->address }},
