...
@if (core()->getConfigData('general.seo.open_graph.enable'))
    <?php
        ...
        $ogDescription = trim($product->meta_description) ?: \Illuminate\Support\Str::limit(trim(strip_tags($product->description)), 190);
                                ...
                                <div class="mt-8 grid max-w-max grid-cols-[auto_1fr] gap-4">
                                    @foreach ($customAttributeValues as $customAttributeValue)
                                        @if (!empty($customAttributeValue['value']))
                                            ...
                                            @else
                                                <div class="grid">
                                                    <p class="text-base text-mutedText">
                    ...
                    <x-slot:header class="bg-gray-100 max-md:!py-3 max-sm:!py-2">
                        ...
                        <x-slot:content class="max-sm:px-0">
                            <div class="container max-1180:px-5">
                                <div
                                    class="grid max-w-max grid-cols-[auto_1fr] gap-4 text-lg text-mutedText max-1180:text-sm">
                                    @foreach ($customAttributeValues as $customAttributeValue)
                                        @if (!empty($customAttributeValue['value']))
                                            <div class="grid">
                                            ...
                                            @else
                                                <div class="grid">
                                                    <p class="text-base text-mutedText" v-pre>
                                                        {{ $customAttributeValue['value'] ?? '-' }}
                                                    </p>
                                                </div>
                                            @endif
                                ...
                                <div class="relative max-w-[590px] max-1180:w-full max-1180:max-w-full max-1180:px-5 max-sm:px-4">
                                    ...
                                    <div class="flex justify-between gap-4">
                                        ...
                                        @if (core()->getConfigData('customer.settings.wishlist.wishlist_option'))
                                            <div
                                                class="max-sm:min-h-7 max-sm:min-w-7 flex max-h-[46px] min-h-[46px] min-w-[46px] cursor-pointer items-center justify-center rounded-full border bg-pageBg text-2xl transition-all hover:opacity-[0.8] max-sm:max-h-7 max-sm:text-base"
                                                role="button"
                                    ...
                                    @if (count($product->getTypeInstance()->getCustomerGroupPricingOffers()))
                                        <div class="mt-2.5 grid gap-1.5">
                                            @foreach ($product->getTypeInstance()->getCustomerGroupPricingOffers() as $offer)
                                                <p class="text-mutedText [&>*]:text-black">
                                                    {!! $offer !!}
                                                </p>
                                            @endforeach
                                        </div>
