...
<div class="panel-side journal-scroll grid max-h-[1320px] min-w-[342px] max-w-[380px] grid-cols-[1fr] gap-8 overflow-y-auto overflow-x-hidden max-xl:min-w-[270px] max-md:max-w-full max-md:gap-5">
    ...
    <div class="grid grid-cols-[auto_1fr] items-center gap-4 rounded-xl border border-divider px-5 py-[25px] max-md:py-2.5">
        <div class="">
            <img
        ...
        >
            ...
            <p class="no-underline max-md:text-md: text-mutedText">
                {{ $customer->email }}
            </p>
        </div>
    </div>
    ...
    @foreach (menu()->getItems('customer') as $menuItem)
        <div>
            ...
            @if ($menuItem->haveChildren())
                <div class="grid rounded-md border border-b border-l-[1px] border-r border-t-0 border-divider max-md:border-none">
                    @foreach ($menuItem->getChildren() as $subMenuItem)
                        <a href="{{ $subMenuItem->getUrl() }}">
                            <div class="flex justify-between px-6 py-5 border-t border-divider hover:bg-subtleBg cursor-pointer max-md:p-4 max-md:border-0 max-md:py-3 max-md:px-0 {{ $subMenuItem->isActive() ? 'bg-subtleBg' : '' }}">
