    ...
    >
        <div class="max-md:max-y-6 flex justify-center gap-6 max-lg:flex-wrap max-md:grid max-md:grid-cols-2 max-md:gap-x-2.5 max-md:text-center">
            @foreach ($customization->options['services'] as $service)
                <div class="flex items-center gap-5 bg-pageBg max-md:grid max-md:gap-2.5 max-sm:gap-1 max-sm:px-2">
                    <span
                        class="{{ $service['service_icon'] }} flex items-center justify-center w-[60px] h-[60px] bg-pageBg border border-black rounded-full text-4xl text-navyBlue p-2.5 max-md:m-auto max-md:w-16 max-md:h-16 max-sm:w-10 max-sm:h-10 max-sm:text-2xl"
                        role="presentation"
                    >
                    </span>
                    ...
                    <div class="max-lg:grid max-lg:justify-center">
                        <!-- Service Title -->
                        <p class="text-base font-medium max-md:text-xl max-sm:text-sm">
                            {{ $service['title'] }}
                        </p>

                        <!-- Service Description -->
                        <p class="mt-2.5 max-w-[217px] text-sm font-medium text-mutedText max-md:mt-0 max-md:text-base max-sm:text-xs">
                            {{ $service['description'] }}
                        </p>
                    </div>
                </div>
